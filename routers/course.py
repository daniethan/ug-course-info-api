...
def create_course(request: schemas_.CourseIn, db: Session=Depends(dbconfig.get_session)):
    return course.create(request, db)


@router.get('/search-course', response_model=List[schemas_.ShowCourse])
def search(*,db: Session=Depends(dbconfig.get_session),q: Union[str,None] = Query(None,title='free query')):
...
async def get_featured_course(uce: schemas_.UCEResultIn, uace: schemas_.UACEResultIn, db: Session=Depends(dbconfig.get_session)):
